...
void gamefield::UpdateValues()
{
    ...
    for(int i = 0;i<4;i++)
    {
        ...
        if((field_val[(block_copy[N][i].y+30+30)/30][(block_copy[N][i].x+30)/30]) == 1 )
            down_coll= true;
        if((field_val[(block_copy[N][i].y)/30][(block_copy[N][i].x+30)/30]) == -1 )
        {
            if((field_val[(block_copy[N][i].y+30)/30][(block_copy[N][i].x+30)/30]) == 1 )
              {gameover = true;
              }
        }
    }
    ...
    system("CLS");
    //std::cout<<can_rotate<<std::endl;
    //for(int i=0;i<22;i++)
    //{
       //for(int j = 0;j<12;j++)
           // {
               // std::cout<<field_val[i][j];
            //}
            //std::cout<<std::endl;
   //}
}
...
void gamefield::UpdateGameField(SDL_Renderer *r,SDL_Event e,SDL_Texture *bg)
{
    ...
    t->BlockEvents(e);
    //std::cout<<left_coll<<" "<<right_coll<<" "<<down_coll<<" "<<N<<" "<<t->Return_n()<<std::endl;
